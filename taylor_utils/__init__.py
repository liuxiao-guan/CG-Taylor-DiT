...
def firstblock_derivative_approximation(
    cache_dic: Dict, current: Dict, feature: torch.Tensor
):
    """
    ...
    """
    difference_distance = (
        current["block_activated_steps"][-1] - current["block_activated_steps"][-2]
    )
...
def firstblock_taylor_formula(cache_dic: Dict, current: Dict) -> torch.Tensor:
    ...
    for i in range(len(cache_dic["cache"]["firstblock_hidden"])):
        ...
        output += (
            (1 / math.factorial(i))
            * cache_dic["cache"]["firstblock_hidden"][i]
            * (x**i)
        )
...
def step_derivative_approximation(
    cache_dic: Dict, current: Dict, feature: torch.Tensor
):
    """
    Compute derivative approximation.
